...
- GGUF files that mix importance-quantized tensors into a K-quant build now load. unsloth's UD-Q4_K_M and UD-Q4_K_XL variants keep most of the feed-forward weights in IQ4_XS, with IQ4_NL and IQ3_S beside them, and were refused at the download step because candle has no name for those types. The engine now reads the header itself, holds those weights in buffers of its own, and multiplies by them with its own kernels: a matvec for decode and the staged TensorOps GEMM for prefill. The remaining IQ types and the ternary ones are still refused before the download.
...
- `oxydllm pull` resumes. A download used to start every file from zero after a sleep, a dropped network or a shutdown, and for a GGUF variant it deleted the files already on disk before starting again; a 16 GB file that stopped at 15 was 15 GB wasted. Each file is now requested from the bytes already on disk with a range request, a connection that goes quiet for a minute or drops mid-body is reopened up to five times from where it stopped, a server that ignores the range starts the file over, a file longer than upstream's is refetched rather than trusted, and a file already of upstream's length is not requested at all. `--force` still starts from zero. Verified against a local server for each case and against Hugging Face on a truncated file.

### New Features
- IQ4_XS, IQ4_NL and IQ3_S GGUF weights (`src/common/gguf_header.rs`, `src/common/iq_quant.rs`, the `mpp_mv_iq*` and `mpp_gemm_iq*_staged` kernels). A header reader of the crate's own replaces candle's for every GGUF read, so a tensor type candle cannot name no longer stops the file; the loader builds such weights as Metal buffers and the quantized linear layer routes them to the engine's kernels, and a token embedding kept in one of these types is gathered row by row by a kernel of its own rather than materialised. All three kernels are checked against a CPU decoder translated from ggml, on random blocks and on blocks taken from a published file. Off Metal, and on a GPU that compiles the kernel library but cannot build its pipelines, these files are refused with the reason named.
- `oxydllm estimate` on a local GGUF prints the file's composition, type by type with the embedding and output types, under the label taken from its name: a UD-Q4_K_M that keeps 117 tensors in IQ4_XS now says so where it used to say only Q4_K_M.
